...
def preprocess_data(df):
    ...
    district_dict = {'ΗΠΕΙΡΟΥ':'Epirus', 'ΗΠΕΙΡΟΥ ': 'Epirus', 'ΚΕΝΤΡΙΚΗΣ ΜΑΚΕΔΟΝΙΑΣ': 'Cent_Macedonia', 'ΚΕΝΤΡΙΚΗΣ ΜΑΚΕΔΟΝΙΑΣ ': 'Cent_Macedonia',  'Κ ΜΑΚΕΔΟΝΙΑΣ': 'Cent_Macedonia',
                     'ΔΥΤΙΚΗΣ ΜΑΚΕΔΟΝΙΑΣ':'West_Macedonia', 'Δ ΜΑΚΕΔΟΝΙΑΣ':'West_Macedonia', 'ΙΟΝΙΩΝ ΝΗΣΙΩΝ': 'Ionian_Islands', 'ΙΟΝΙΩΝ ΝΗΣΩΝ' : 'Ionian_Islands', 
                     'ΣΤΕΡΕΑΣ ΕΛΛΑΔΟΣ': 'Cent_Greece',  'ΣΤΕΡΕΑΣ ΕΛΛΑΔΟΣ ': 'Cent_Greece', 'ΣΤΕΡΕΑΣ ΕΛΛΑΔΑΣ': 'Cent_Greece', 'ΣΤ ΕΛΛΑΔΑΣ': 'Cent_Greece', 'ΚΡΗΤΗΣ': 'Crete',
                     'ΒΟΡΕΙΟΥ ΑΙΓΑΙΟΥ': 'North_Aegean', 'ΘΕΣΣΑΛΙΑΣ':'Thessaly',  'ΝΟΤΙΟΥ ΑΙΓΑΙΟΥ':'South_Aegean','ΔΥΤΙΚΗΣ ΕΛΛΑΔΟΣ':'West_Greece','Δ ΕΛΛΑΔΑΣ' :'West_Greece', 
                     'ΔΥΤΙΚΗΣ ΕΛΛΑΔΟΣ ':'West_Greece', 'ΔΥΤΙΚΗΣ ΕΛΛΑΔΑΣ' :'West_Greece', 'ΠΕΛΟΠΟΝΝΗΣΟΥ':'Peloponnese', 'ΠΕΛΟΠΟΝΝΗΣΟΥ             ':'Peloponnese', 'ΑΤΤΙΚΗΣ': 'Attica',
                     'ΑΤΤΙΚΗΣ ': 'Attica', 'ΑΝΑΤΟΛΙΚΗΣ ΜΑΚΕΔΟΝΙΑΣ ΚΑΙ ΘΡΑΚΗΣ' :'East_Macedonia_Thrace','ΑΝΑΤΟΛΙΚΗΣ ΜΑΚΕΔΟΝΙΑΣ & ΘΡΑΚΗΣ':'East_Macedonia_Thrace',
                     'ΑΝ ΜΑΚΕΔΟΝΙΑΣ ΘΡΑΚΗΣ':'East_Macedonia_Thrace', 'ΑΝ. ΜΑΚΕΔΟΝΙΑΣ & ΘΡΑΚΗΣ':'East_Macedonia_Thrace', 'ΣΤΕΡΕΑΣ ΕΛΛΑΔΟΣ - ΘΕΣΣΑΛΙΑΣ': 'Cent_Greece_Thessaly',
                     'ΣΤΕΡΕΑΣ ΕΛΛΑΔΟΣ & ΘΕΣΣΑΛΙΑΣ': 'Cent_Greece_Thessaly', 'ΘΕΣΣΑΛΙΑΣ,ΣΤΕΡΕΑΣ ΕΛΛΑΔΑΣ': 'Cent_Greece_Thessaly','ΔΥΤΙΚΗΣ ΕΛΛΑΔΟΣ -  ΚΕΝΤΡΙΚΗΣ  ΜΑΚΕΔΟΝΙΑΣ':'West_Greece_Cent_Macedonia',
                     'ΘΕΣΣΑΛΙΑΣ & ΔΥΤΙΚΗΣ ΜΑΚΕΔΟΝΙΑΣ': 'Thessaly_West_Macedonia', 'ΔΥΤΙΚΗΣ ΜΑΚΕΔΟΝΙΑΣ,ΘΕΣΣΑΛΙΑΣ': 'Thessaly_West_Macedonia', 'ΘΕΣΣΑΛΙΑΣ,ΔΥΤΙΚΗΣ ΜΑΚΕΔΟΝΙΑΣ': 'Thessaly_West_Macedonia', 
                     'ΔΥΤΙΚΗΣ ΕΛΛΑΔΟΣ & ΣΤΕΡΕΑΣ ΕΛΛΑΔΟΣ':'West_Greece_Cent_Greece', 'ΣΤΕΡΕΑΣ ΕΛΛΑΔΟΣ  -  ΑΤΤΙΚΗΣ' : 'Attica_Cent_Greece', 'ΑΤΤΙΚΗΣ,ΣΤΕΡΕΑΣ ΕΛΛΑΔΑΣ' : 'Attica_Cent_Greece',
                     'ΔΥΤΙΚΗΣ ΜΑΚΕΔΟΝΙΑΣ,ΚΕΝΤΡΙΚΗΣ ΜΑΚΕΔΟΝΙΑΣ': 'West_Macedonia_Cent_Macedonia','ΘΕΣΣΑΛΙΑΣ,ΗΠΕΙΡΟΥ': 'Thessaly_Epirus', 'ΑΝ. ΜΑΚΕΔΟΝΙΑΣ & ΘΡΑΚΗΣ,ΚΕΝΤΡΙΚΗΣ ΜΑΚΕΔΟΝΙΑΣ': "East_Macedonia_Thrace_Central_Macedonia"
                    }
...
def transform_data_on_time_level(df, cols_to_drop_list, start_time_col, time_period):
    ...
    aggregation_funcs = {col: 'mean' for col in df_period.columns if col not in [start_time_col, 'max_mp', 'num_permits']}
    ...
    aggr.rename(columns={col: f'avg_{col}' for col in aggregation_funcs if col not in [start_time_col, 'max_mp', 'num_permits']}, inplace=True)
...
def main(): 
    ...
    if freq == 'Weekly':
        cols_to_drop_list = [ 'start_production_month', 'approval_period_in_months', 'approved_time_in_months',  'monthly_co2_price']
        time_col = 'start_production_week'
        time_period = 'W'
    else:
        cols_to_drop_list = ['start_production_week', 'approval_period_in_weeks', 'approved_time_in_weeks',  'weekly_co2_price']
